...
function Drawer({
  ...
  const [filter, setFilter] = useLocalStorage<string | "Todas">(
    `nebula.filter.${eventType}`,
    "Todas"
  );
  const [last, setLast] = useState<(typeof EVENT_CARDS)[0] | null>(null);

  const allCards = useMemo(
    () => EVENT_CARDS.filter((c) => c.type === eventType),
    [eventType]
  );

  const categories = useMemo(
    () => ["Todas", ...Array.from(new Set(allCards.map((c) => c.category)))],
    [allCards]
  );
  ...
  const pool = useMemo(
    () =>
      filter === "Todas"
        ? allCards
        : allCards.filter((c) => c.category === filter),
    [filter, allCards]
  );
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2">
          <select
            ...
            className="bg-slate-800 border border-slate-600 rounded-xl px-3 py-2 text-white/95 focus:outline-none focus:ring-2 focus:ring-cyan-500"
          >
            {categories.map((c) => (
